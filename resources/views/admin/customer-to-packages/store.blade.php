...
@section('title', 'Assign Package - Store')
...
<div class="container-fluid">
    <div class="row mb-4">
        ...
        <div class="col-auto">
            <a href="{{ route('admin.customer-to-packages.index') }}" class="btn btn-outline-secondary">
    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="assignPackageForm" action="{{ route('admin.customer-to-packages.store') }}" method="POST">
                        @csrf
                                    ...
                                    <div class="package-row mb-3" data-index="0">
                                        <div class="row g-2 align-items-end">
                                            <div class="col-md-5">
                                                ...
                                                <select class="form-select package-select @error('packages.0.package_id') is-invalid @enderror"
                                                        name="packages[0][package_id]" data-index="0" required>
                                                    <option value="">Select a package...</option>
                                                    @foreach($packages as $package)
                                                        <option value="{{ $package->p_id }}"
                                                                data-price="{{ $package->monthly_price }}"
                                                                data-type="{{ $package->package_type }}"
                                                                {{ old('packages.0.package_id') == $package->p_id ? 'selected' : '' }}>
                                                            {{ $package->name }} - ৳{{ number_format($package->monthly_price,2) }}/month
                                                            ({{ ucfirst($package->package_type) }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                                @error('packages.0.package_id')
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select billing-months @error('packages.0.billing_cycle_months') is-invalid @enderror"
                                                        name="packages[0][billing_cycle_months]" data-index="0" required>
                                                    <option value="1" {{ old('packages.0.billing_cycle_months','1')=='1'?'selected':'' }}>1 Month</option>
                                                    <option value="2" {{ old('packages.0.billing_cycle_months')=='2'?'selected':'' }}>2 Months</option>
                                                    <option value="3" {{ old('packages.0.billing_cycle_months')=='3'?'selected':'' }}>3 Months</option>
                                                    <option value="6" {{ old('packages.0.billing_cycle_months')=='6'?'selected':'' }}>6 Months</option>
                                                    <option value="12" {{ old('packages.0.billing_cycle_months')=='12'?'selected':'' }}>12 Months</option>
                                                </select>
                                                @error('packages.0.billing_cycle_months')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="date" class="form-control assign-date @error('packages.0.assign_date') is-invalid @enderror"
                                                       name="packages[0][assign_date]"
                                                       value="{{ old('packages.0.assign_date', date('Y-m-d')) }}" data-index="0" required>
                                                @error('packages.0.assign_date')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="col-md-1">
                                                ...
                                                <button type="button" class="btn btn-outline-danger btn-sm w-100 remove-package-btn" disabled>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('addPackageBtn').addEventListener('click', function () {
        ...
        row.innerHTML = `
            <div class="row g-2 align-items-end">
                <div class="col-md-5">
                    ...
                    <select class="form-select package-select" name="packages[${idx}][package_id]" data-index="${idx}" required>
                        <option value="">Select a package...</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Billing Months *</label>
                    <select class="form-select billing-months" name="packages[${idx}][billing_cycle_months]" data-index="${idx}" required>
                ...
                <div class="col-md-3">
                    <label class="form-label">Assign Date *</label>
                    <input type="date" class="form-control assign-date" name="packages[${idx}][assign_date]"
        ...
        select.addEventListener('change', () => { updateSelectedPackages(); calculatePackageAmount(idx); });
        row.querySelector('.billing-months').addEventListener('change', () => calculatePackageAmount(idx));
    ...
    function updateSelectedPackages() {
        ...
        document.querySelectorAll('.package-select').forEach(s => s.value && selectedPackages.add(s.value));
        updatePackageOptions();
    }
    ...
    function calculatePackageAmount(idx) {
        ...
        if (sumEl) sumEl.innerHTML = `<span>Package ${+idx + 1}:</span><span>৳ ${total.toLocaleString()}</span>`;
        calculateTotal();
    }
    ...
    // ---------- 8. Initial events ----------
    document.querySelector('.package-select[data-index="0"]').addEventListener('change', () => { updateSelectedPackages(); calculatePackageAmount(0); });
    document.querySelector('.billing-months[data-index="0"]').addEventListener('change', () => calculatePackageAmount(0));
    document.querySelector('.remove-package-btn').addEventListener('click', () => removePackage(0));
    ...
    function updateSubmitButton() {
        ...
        const hasPackages = document.querySelectorAll('.package-select[value!=""]').length > 0;
        submitBtn.disabled = !(hasCustomer && hasPackages);
    }
