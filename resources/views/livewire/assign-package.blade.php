    ...
    <div class="card">
        ...
        <div class="card-body">
            <form wire:submit.prevent="submit">
                ...
                <!-- Package Selection with Dynamic Adding/Removing -->
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <div class="packages-container">
                            @foreach($rows as $index)
                                <div class="package-row mb-3" wire:key="package-row-{{ $index }}">
                                    <div class="row g-2 align-items-end">
                                        <div class="col-md-5">
                                            <label class="form-label">Package {{ $loop->iteration }} *</label>
                                            <select class="form-select package-select @error('packageSelections.' . $index) is-invalid @enderror" 
                                                    wire:model="packageSelections.{{ $index }}"
                                        ...
                                        <div class="col-md-1">
                                            ...
                                            <div class="package-amount">
                                                ৳ {{ number_format($this->getPackageAmount($index), 2) }}
                                            </div>
                                        </div>
                                        <div class="col-md-1">
                        ...
                        @if(count($rows) === 0)
                            <div class="alert alert-warning text-center">
                                ...
                                Please add at least one package to assign.
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="summary-card">
                            ...
                            <div class="summary-details">
                                @foreach($rows as $index)
                                    @if(!empty($packageSelections[$index]))
                                        @php
                                            $packageAmount = $this->getPackageAmount($index);
                                        @endphp
                                        @if($packageAmount > 0)
                                            <div class="summary-row">
                                                <span>
                                                    <i class="fas fa-cube me-2"></i>
                                                    Package {{ $loop->iteration }} ({{ $billingMonths[$index] ?? 1 }} month{{ $billingMonths[$index] > 1 ? 's' : '' }})
                                                </span>
                                                <span class="fw-bold text-success">৳ {{ number_format($packageAmount, 2) }}</span>
                                            </div>
                                        @endif
                                    @endif
                                @endforeach
                    ...
                    <div class="col-12">
                        <button type="submit" class="btn btn-success btn-lg w-100 py-3" 
                            ...
                            <div wire:loading.remove wire:target="submit">
                                <i class="fas fa-check-circle me-2"></i>Assign Packages
                            </div>
                        ...
                        @elseif($totalAmount === 0)
                            <div class="text-center text-muted small mt-2">
                                <i class="fas fa-info-circle me-1"></i>Please select at least one package to assign
                            </div>
                        @endif
                    </div>
                </div>
