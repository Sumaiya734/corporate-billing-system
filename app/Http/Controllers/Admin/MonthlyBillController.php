...
class MonthlyBillController extends Controller
{
    ...
    private function getAllActiveCustomersWithProducts(Carbon $monthDate)
    {
        return DB::table('customers as c')
            ->select(
                ...
                'c.email',
                'c.phone',
                DB::raw('SUM(CASE WHEN cp.is_custom_price = 1 AND cp.custom_price > 0 THEN cp.custom_price ELSE 0 END) as total_product_amount'),
                DB::raw('GROUP_CONCAT(CONCAT(p.p_id, ":", p.monthly_price, ":", cp.billing_cycle_months, ":", cp.cp_id, ":", COALESCE(cp.custom_price, "NULL"), ":", COALESCE(cp.is_custom_price, 0))) as product_details')
            )
            ...
            ->map(function($customer) {
                ...
                if ($customer->product_details) {
                    ...
                    foreach ($products as $product) {
                        if (strpos($product, ':') !== false) {
                            $productParts = explode(':', $product);
                            if (count($productParts) >= 6) {
                                list($p_id, $monthly_price, $cycle, $cp_id, $custom_price, $is_custom_price) = $productParts;
                                $productDetails[] = [
                                    'p_id' => $p_id,
                                    'cp_id' => $cp_id,
                                    'monthly_price' => $monthly_price,
                                    'billing_cycle_months' => $cycle,
                                    'custom_price' => $custom_price === 'NULL' ? null : $custom_price,
                                    'is_custom_price' => (bool) $is_custom_price
                                ];
                            }
                        }
                    }
    ...
    private function createCustomerMonthlyInvoice($customer, Carbon $monthDate)
    {
        ...
        foreach (($customer->product_details ?? []) as $product) {
            ...
            // Only calculate product amount if customer should be billed this month AND has custom price
            $productAmount = 0;
            if ($shouldBillThisMonth) {
                // ONLY use custom_price - no calculated price fallback
                if (isset($product['is_custom_price']) && $product['is_custom_price'] && 
                    isset($product['custom_price']) && $product['custom_price'] !== null && $product['custom_price'] > 0) {
                    $productAmount = $product['custom_price'];
                }
                // If no custom price is set, don't bill anything (productAmount remains 0)
            }
            ...
            // Always create invoice for active customers to ensure visibility in all months
            // Even if there's no current billing or carried forward amount
            $invoice = Invoice::create([
                'cp_id' => $product['cp_id'],
                'issue_date' => $monthDate->format('Y-m-d'),
                'previous_due' => $previousDue,
    ...
    private function calculateMonthlyAmounts($assignDate, $billingCycle, $subtotalAmount, $targetMonth) {
        ...
        if ($monthsSinceAssign == 0) {
            ...
            // Previous due should be based on actual unpaid amounts, not multiplication of cycles
            // For now, we'll set it to 0 as this method should only calculate theoretical amounts
            // Actual previous due will be determined from existing invoices
            $previousDue = 0;
            ...
            // This should be based on actual previous invoices, not calculated theoretically
            // For now, we'll set appropriate defaults
            $totalAmount = $subtotalAmount; // In carry-forward months, total should match the base subtotal
            
            return [
                'subtotal' => 0,
                'previous_due' => $totalAmount, // This will be adjusted based on actual previous invoices
                'total_amount' => $totalAmount,
                'cycle_number' => $cycleNumber,
    ...
    public function closeMonth(Request $request)
    {
        ...
        try {
            ...
            foreach ($allInvoices as $invoice) {
                ...
                // Create invoice for next month to ensure customer continues to appear in subsequent months
                ...
                if ($existingNextInvoice) {
                    // For existing invoices, ensure they're consistent
                    ...
                    if ($customerProduct) {
                        ...
                        if ($isBillingMonth) {
                            // Always use custom price if available and is_custom_price is true
                                                        ...
                                                        // If no custom price is set, don't bill anything (newSubtotal remains 0)
                        }
                        
                        // Create invoice with zero amounts for visibility
                        $newInvoice = Invoice::create([
                            ...
                            'received_amount' => 0,
                            'next_due' => $newSubtotal,
                            'status' => $newSubtotal > 0 ? 'unpaid' : 'paid',
                            'notes' => "Zero amount invoice for visibility of customer in next month",
                            'created_by' => \Illuminate\Support\Facades\Auth::id()
                        ]);
        ...
        try {
            ...
            if ($dueAmount > 0) {
                ...
                $invoice->update([
                    ...
                    'next_due' => max(0, $invoice->total_amount - $invoice->received_amount), // ✅ Calculate from actual values
                    'notes' => ($invoice->notes ?? '') . $closedNote,
                    'is_closed' => true,
                    'closed_at' => now(),
                    'closed_by' => \Illuminate\Support\Facades\Auth::id()
                ]);
                // Recalculate payment amounts to ensure consistency
                $invoice->recalculatePaymentAmounts();
                ...
                if ($existingNextInvoice) {
                    ...
                    if (!$alreadyAdded) {
                        ...
                        // Preserve the existing subtotal (including custom_price)
                        ...
                        // Ensure the updated invoice is consistent
                        $existingNextInvoice->refresh();
                        ...
                        Log::info('Prevented duplicate carry-forward for invoice', [
                            'invoice_id' => $invoice->invoice_id,
                    ...
                    // For non-billing months, subtotal should be 0
                    
                    // Get customer product details to determine if next month is a billing month
                    $customerProduct = CustomerProduct::find($request->cp_id);
                    ...
                    $newInvoice = Invoice::create([
                        ...
                        'subtotal' => $newSubtotal, // Only add subtotal for billing months
                        'total_amount' => $newTotalAmount, // Subtotal + carried forward amount
                        'received_amount' => 0,
                        'next_due' => $newTotalAmount, // total_amount - received_amount (0) = $newTotalAmount
                        'status' => $newSubtotal > 0 ? 'unpaid' : ($dueAmount > 0 ? 'unpaid' : 'paid'),
                        'notes' => "Carried forward amount from invoice {$invoice->invoice_number}" . ($newSubtotal > 0 ? " (Billing cycle month)" : " (Carry-forward month)"),
                        'created_by' => \Illuminate\Support\Facades\Auth::id()
                    ]);
                    
                    // Ensure the new invoice is consistent
                    $newInvoice->refresh();
                ...
                // Fully paid, mark as closed and create invoice for next month to ensure visibility
                $invoice->update([
                    'status' => 'paid',
                    'is_closed' => true,
                    'closed_at' => now(),
                ...
                if (!$existingNextInvoice) {
                    ...
                    if ($customerProduct) {
                        ...
                        Invoice::create([
                            ...
                            'next_due' => $newSubtotal,
                            'status' => $newSubtotal > 0 ? 'unpaid' : 'paid',
                            'notes' => "Zero amount invoice for visibility of fully paid customer",
                            'created_by' => \Illuminate\Support\Facades\Auth::id()
                        ]);
                    }
                }
            ...
            // Get information about the next month's invoice for the response
            $nextMonthInvoice = null;
            if ($dueAmount > 0) {
                // Use immediate next month instead of next billing cycle month
                $nextMonth = Carbon::parse($invoice->issue_date)->addMonth();
                $nextMonthInvoice = Invoice::where('cp_id', $request->cp_id)
                    ->whereYear('issue_date', $nextMonth->year)
                    ->whereMonth('issue_date', $nextMonth->month)
                    ->first();
            }
            ...
            // Get the next cycle's subtotal from the invoice
            $nextCycleSubtotal = $invoice->subtotal ?? 0;
            ...
            return response()->json([
                ...
                'next_month_invoice' => $nextMonthInvoice ? [
                    'invoice_id' => $nextMonthInvoice->invoice_id,
                    'invoice_number' => $nextMonthInvoice->invoice_number,
                    'issue_date' => $nextMonthInvoice->issue_date,
                    'total_amount' => $nextMonthInvoice->total_amount,
                    'next_due' => $nextMonthInvoice->next_due,
                ] : null,
                'next_month_url' => $dueAmount > 0 ? route('admin.billing.monthly-bills', [
    ...
    /**
     * Implement proper sequential carry-forward logic
     * Each month carries forward its next_due to the next month's previous_due
     * This ensures a clean sequential chain: Jan->Feb->Mar->Apr->May->Jun->Jul
     */
    private function propagateCarryForwardToCurrentCycle($invoice, $dueAmount)
    {
        // For proper sequential carry-forward, we don't need to propagate to all months in cycle
        // Each month should naturally get its carry-forward from the previous month during invoice generation
